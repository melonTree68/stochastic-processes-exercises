...
1. 仓库地址：`https://github.com/yuchen-he00/stochastic-process-exercises`
2. 先点击 GitHub 页面右上角 **Fork**。
3. Fork到你自己的仓库后，把你自己的仓库克隆到本地：`git clone <你的仓库地址>`
4. 进入项目目录：`cd stochastic-process-exercises`
5. 把原仓库设为 `upstream`（只需一次）：
    `git remote add upstream https://github.com/yuchen-he00/stochastic-process-exercises.git`
6. 检查远程是否配置成功：`git remote -v`
7. 开始做题前，先同步到最新主分支：
    `git fetch upstream`
    `git checkout main`
...
### 提交 Pull Request 🚀
确认内容修改无误后，在终端执行以下命令将修改提交到你刚刚创建的分支，并推送到你自己的 GitHub 仓库：
...
6. 回到 GitHub，进入你 Fork 的仓库主页，点击 **Compare & pull request**，确认合并路径无误：
    - base repository: `yuchen-he00/stochastic-process-exercises`
    - base branch: `main`
    - compare branch: `your-branch-name`
7. 在 PR 描述中简要写明做了哪些修改，然后点击 Create pull request 按钮完成提交。。
